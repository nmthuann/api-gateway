...
export class AuthenticationMiddleware implements NestMiddleware {
  constructor(private readonly jwtService: JwtService, @Inject(CACHE_MANAGER) private readonly cacheService: Cache) {}
  ...
  async use(req: Request, res: Response, next: NextFunction) {
    ...
    if (typeof authHeader === 'undefined') {
      throw new HttpException('Middleware - Unauthorized: 401 - header empty!', HttpStatus.UNAUTHORIZED)
    } else {
      //   cut token
      ...
      try {
        ...
        if (typeof checkExpired !== 'undefined') {
          const getValueByKeyInRedis: string = await this.cacheService.get(checkExpired['payload']['email'])
          ...
          if (getValueByKeyInRedis) {
            ...
            if (response == null) {
              res.status(HttpStatus.FORBIDDEN).json({ message: 'refresh token null - Please Login again!' })
            } else {
              //  res not null -> stay logged in!
            ...
            res.status(HttpStatus.FORBIDDEN).json({ message: 'Please Login again - decode fail' })
          }
        } else {
