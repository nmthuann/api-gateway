...
import { ProfileDocumentDto } from '../users/dtos/profile.document.dto';
import { OrderDetailDto } from './dtos/order-detail.dto';
import { OrderDetailCustomerDto } from './dtos/order-detail-customer.dto';
...
export class ApiGatewayOrderService {
  ...
  async getProfileByFreelancerId(): Promise<ProfileDocumentDto> {
  ...
  async createOrder(getOrder: GetOrderDto): Promise<OrderDto> {
    ...
    orderDto.totalPrice =
      getPostById.post_detail.packages[
        getOrder.package_id
      ].package_detail.unit_price;
    orderDto.deliveryTime =
      getPostById.post_detail.packages[
        getOrder.package_id
      ].package_detail.delivery_day;
  ...
  async getOrderDetailCustomer(
    ...
    if (!getOrderByCustomerId) return null;
      ...
      const getPostById = await this.getPostById(
        getOrderByCustomerId.jobPostID,
      );
      const getProfile = await this.getProfileByEmail(
        getOrderByCustomerId.FreelancerID,
      ); // find profile
      ...
      orderDetailDto.package_name =
        getPostById.post_detail.packages[
          getOrderByCustomerId.packageDetailID
        ].package_name;
      ...
      orderDetailDto.freelancer_name =
  ...
  async getOrderDetailsByCustomerId(email: string): Promise<OrderDetailDto[]> {
    const getOrdesByCustomerId: OrderDto[] = await this.findOrdersByCustomerId(
      email,
    );
    const orderDetailList: OrderDetailDto[] = [];
    ...
    for (const order of getOrdesByCustomerId) {
      ...
      const getProfile = await this.getProfileByEmail(
        getPostById.post_detail.profile_user,
      );
      ...
      const packageDetail =
        getPostById.post_detail.packages[order.packageDetailID];
  ...
  async getOrderDetailByFreelancerId(
    id: number,
    email: string,
  ): Promise<OrderDetailDto> {
    const orderDetailDto: OrderDetailDto = {
  ...
  async getOrderDetailsByFreelancerId(
    email: string,
  ): Promise<OrderDetailCustomerDto[]> {
    const getOrdersByFreelancerId: OrderDto[] =
      await this.findOrdersByFreelancerId(email);
    const orderDetailList: OrderDetailCustomerDto[] = [];
    for (const order of getOrdersByFreelancerId) {
      const orderDetailDto: OrderDetailCustomerDto = {
        post_name: '',
        post_id: 0,
        description: '',
        package_name: '',
