...
export class ApiGatewayOrderController {
  ...
  async getOrderDetailByFreelancer(
    @Param('id') id: number,
    @Request() req: any,
  ): Promise<OrderDetailDto> {
    const email = req['email'];
    return await this.apiGatewayOrderService.getOrderDetailByFreelancerId(
      id,
      email,
    );
  }
  ...
  async getOrderDetailsByFreelancerId(
    ...
    const email = req['email'];
    return await this.apiGatewayOrderService.getOrderDetailsByFreelancerId(
      email,
    );
  }
}
